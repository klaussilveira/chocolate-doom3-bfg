...
#define SWF_TEXT_PTHIS_FUNC(x)                                            \
    idSWFTextInstance* pThis = thisObject ? thisObject->GetText() : NULL; \
    if (!verify(pThis != NULL)) {                                         \
        idLib::Warning("SWF: tried to call " x " on NULL edittext");      \
        return idSWFScriptVar();                                          \
    }
#define SWF_TEXT_PTHIS_GET(x)                                     \
    idSWFTextInstance* pThis = object ? object->GetText() : NULL; \
    if (pThis == NULL) {                                          \
        return idSWFScriptVar();                                  \
    }
#define SWF_TEXT_PTHIS_SET(x)                                     \
    idSWFTextInstance* pThis = object ? object->GetText() : NULL; \
    if (pThis == NULL) {                                          \
        return;                                                   \
    }

#define SWF_TEXT_FUNCTION_SET(x) \
