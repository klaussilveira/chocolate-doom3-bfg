...
int idParser::MergeTokens(idToken* t1, idToken* t2)
{
    ...
    if (t1->type == TT_NUMBER && t2->type == TT_NUMBER && !(t1->subtype & (TT_HEX | TT_BINARY)) && !(t2->subtype & (TT_HEX | TT_BINARY)) && (!(t1->subtype & TT_FLOAT) || !(t2->subtype & TT_FLOAT))) {
        t1->Append(t2->c_str());
        return true;
    }
...
void idParser::SetIncludePath(const char* path)
{
    ...
    if (idParser::includepath[idParser::includepath.Length() - 1] != '\\' && idParser::includepath[idParser::includepath.Length() - 1] != '/') {
