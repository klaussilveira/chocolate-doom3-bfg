...
static void designfir(float* g, float fc)
{
    ...
    beta = (float)(exp(log((double)0.58417 * (att - 20.96)) * 0.4) + 0.07886 * (att - 20.96));
...
static void filter(sample_t* result, sample_t* data, int32_t length, float coef[])
{
    ...
    for (sample = 0; sample < ORDER2; sample++) {
        ...
        for (i = 0; i < ORDER; i++)
            sum += (float)(coef[i] * ((sample_window < 0) ? 0.0 : data[sample_window++]));
    ...
    for (sample = length - ORDER + ORDER2; sample < length; sample++) {
        ...
        for (i = 0; i < ORDER; i++)
            sum += (float)(coef[i] * ((sample_window >= length) ? 0.0 : data[sample_window++]));
