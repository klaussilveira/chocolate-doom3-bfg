...
static int update_envelope(int v)
{
    ...
    if (((voice[v].envelope_increment < 0) && (voice[v].envelope_volume <= voice[v].envelope_target)) || ((voice[v].envelope_increment > 0) && (voice[v].envelope_volume >= voice[v].envelope_target))) {
...
static void update_tremolo(int v)
{
    ...
    voice[v].tremolo_volume = (float)(1.0 - FSCALENEG((sine(voice[v].tremolo_phase >> RATE_SHIFT) + 1.0) * depth * TREMOLO_AMPLITUDE_TUNING, 17));
