...
static int32_t convert_vibrato_sweep(uint8_t sweep, int32_t vib_control_ratio)
{
    ...
    return (int32_t)(FSCALE((double)(vib_control_ratio)*SWEEP_TUNING, SWEEP_SHIFT)
    ...
    int strip_tail)
{
    ...
    if ((239 != fp->Read(tmp, 239)) || (memcmp(tmp, "GF1PATCH110\0ID#000002", 22) && memcmp(tmp, "GF1PATCH100\0ID#000002", 22))) /* don't know what the
                                                                                                                                                                             differences are */
    {
...
#define READ_CHAR(thing)            \
    if (1 != fp->Read(&tmpchar, 1)) \
        goto fail;                  \
    thing = tmpchar;
#define READ_SHORT(thing)            \
    if (2 != fp->Read(&tmpshort, 2)) \
        goto fail;                   \
    thing = LE_SHORT(tmpshort);
#define READ_LONG(thing)            \
    if (4 != fp->Read(&tmplong, 4)) \
        goto fail;                  \
    ...
    for (i = 0; i < 128; i++) {
        if (bank->tone[i].instrument == MAGIC_LOAD_INSTRUMENT) {
            if (!(bank->tone[i].name)) {
                ...
                bank->tone[i].instrument = 0;
                errors++;
            } else if (!(bank->tone[i].instrument = load_instrument(bank->tone[i].name,
                             (dr) ? 1 : 0,
                             bank->tone[i].pan,
                             bank->tone[i].amp,
