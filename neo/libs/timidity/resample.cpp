...
#define RESAMPLATION                      \
    ...
    *dest++ = v1 + (iplookup[(((v2 - v1) << 5) & 0x03FE0) | ((ofs & FRACTION_MASK) >> (FRACTION_BITS - 5))]);
#else
#define RESAMPLATION                      \
    v1 = src[ofs >> FRACTION_BITS];       \
    v2 = src[(ofs >> FRACTION_BITS) + 1]; \
...
static int32_t update_vibrato(Voice* vp, int sign)
{
    ...
    a = FSCALE(((double)(vp->sample->sample_rate) * (double)(vp->frequency)) / ((double)(vp->sample->root_freq) * (double)(play_mode->rate)),
        FRACTION_BITS);

    pb = (int)((sine(vp->vibrato_phase * (SINE_CYCLE_LENGTH / (2 * VIBRATO_SAMPLE_INCREMENTS)))
...
static sample_t* rs_vib_plain(int v, int32_t* countptr)
{
    ...
    int32_t
        le
        = vp->sample->data_length,
        ofs = vp->sample_offset,
        incr = vp->sample_increment,
        count = *countptr;
    int
        cc
        = vp->vibrato_control_counter;
...
void pre_resample(Sample* sp)
{
    ...
    a = ((double)(sp->sample_rate) * freq_table[(int)(sp->note_to_use)]) / ((double)(sp->root_freq) * play_mode->rate);
    ...
    while (--count) {
        ...
        *dest++ = (int16_t)(v2 + (xdiff / 6.0) * (-2 * v1 - 3 * v2 + 6 * v3 - v4 + xdiff * (3 * (v1 - 2 * v2 + v3) + xdiff * (-v1 + 3 * (v2 - v3) + v4))));
        ofs += incr;
    }
