...
{
    double a;
    a = (double)(tempo) * (double)(play_mode->rate) * (65536.0 / 1000000.0) / (double)(divisions);
...
#define MIDIEVENT(at, t, ch, pa, pb)                                   \
    ...
    newEventList->event.time = at;                                     \
    newEventList->event.type = t;                                      \
    newEventList->event.channel = ch;                                  \
    newEventList->event.a = pa;                                        \
    newEventList->event.b = pb;                                        \
    newEventList->next = 0;                                            \
    return newEventList;
