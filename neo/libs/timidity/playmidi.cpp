...
static void recompute_freq(int v)
{
    int
        sign
        = (voice[v].sample_increment < 0), /* for bidirectional loops */
    ...
    if (pb == 0x2000 || pb < 0 || pb > 0x3FFF)
        ...
        if (!(channel[voice[v].channel].pitchfactor)) {
            ...
            if (pb < 0)
                i = -i;
            channel[voice[v].channel].pitchfactor = (float)(bend_fine[(i >> 5) & 0xFF] * bend_coarse[i >> 13]);
        }
        if (pb > 0)
            voice[v].frequency = (int32_t)(channel[voice[v].channel].pitchfactor * (double)(voice[v].orig_frequency));
        else
            voice[v].frequency = (int32_t)((double)(voice[v].orig_frequency) / channel[voice[v].channel].pitchfactor);
    }

    a = FSCALE(((double)(voice[v].sample->sample_rate) * (double)(voice[v].frequency)) / ((double)(voice[v].sample->root_freq) * (double)(play_mode->rate)),
        FRACTION_BITS);
...
static void recompute_amp(int v)
{
    ...
    if (!(play_mode->encoding & PE_MONO)) {
        if (voice[v].panning > 60 && voice[v].panning < 68) {
            ...
            voice[v].left_amp = FSCALENEG((double)(tempamp)*voice[v].sample->volume * master_volume,
                21);
