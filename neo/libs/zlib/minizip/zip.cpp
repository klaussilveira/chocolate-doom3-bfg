...
DG: The file was renamed from zip.c to zip.cpp so we can use C++ functions from Doom3, furthermore we use doom3 headers
    instead of system headers and Doom3's Mem_Alloc() and Mem_Free() instead of malloc() and free()
    Other than that, there are no real changes to the original file.
...
local uLong zip64local_TmzDateToDosDate(const tm_zip* ptm)
{
    ...
    if (year >= 1980)
        ...
        year -= 80;
    return (uLong)(((ptm->tm_mday) + (32 * (ptm->tm_mon + 1)) + (512 * year)) << 16) | ((ptm->tm_sec / 2) + (32 * ptm->tm_min) + (2048 * (uLong)ptm->tm_hour));
}
...
extern zipFile ZEXPORT zipOpen3(const void* pathname, int append, zipcharpc* globalcomment, zlib_filefunc64_32_def* pzlib_filefunc64_32_def)
{
    ...
    ziinit.filestream = ZOPEN64(ziinit.z_filefunc,
        pathname,
        (append == APPEND_STATUS_CREATE) ? (ZLIB_FILEFUNC_MODE_READ | ZLIB_FILEFUNC_MODE_WRITE | ZLIB_FILEFUNC_MODE_CREATE) : (ZLIB_FILEFUNC_MODE_READ | ZLIB_FILEFUNC_MODE_WRITE | ZLIB_FILEFUNC_MODE_EXISTING));
...
extern int ZEXPORT zipWriteInFileInZip(zipFile file, const void* buf, unsigned int len)
{
    ...
    // DG: cast void pointer to const unsigned char* because crc32() expects that
    //     and C++ is more strict in these things than plain C
