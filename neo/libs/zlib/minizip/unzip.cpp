...
The file was renamed from unzip.c to unzip.cpp so we can use C++ functions from doom3
 ...
 Copyright (C) 2012-2013 Daniel Gibson
...
#if 0
    // DG: if we wanna use idFile_Cached like the original code in D3 BFG did
    // we may need this code
    idFile_Cached *icFile = (idFile_Cached *)us.filestream;
    icFile->CacheData( us.offset_central_dir, us.size_central_dir );
    // DG end
#endif
...
#if 0
    // DG: this code was in the original d3 bfg source from idsoftware, most probably just for those debug cvars?
    // it can't simply be uncommented but would have to be adapted for minizip 1.1 first
 ...
 * Note from Daniel:
 *
 * Doom3 BFG had some further changes (probably for performance) that I won't integrate
 * right now as they're not needed for functionality:
 *
 * * uses idFile_Cached instead of FILE
 *   => in unzOpen and unzReOpen fileSystem->OpenExplicitPakFile( path ); was used
 *   => fileSystem->CloseFile( fin ); was used to close files
 *   => fin->Read instead of fread, fin->Tell instead of ftell etc
 *   ==> all these things can be done with a custom zlib_filefunc_def (or maybe zlib_filefunc64_def)
 *
 * * at the end of unzOpen: us.file->CacheData( us.offset_central_dir, us.size_central_dir );
 *   => this can't be done with a zlib_filefunc_def obviously
 * * some CVars (for debugging?) that I put in a #if 0 - block
 * * some code in unz64local_GetCurrentFileInfoInternal() that I put into an #if 0 - block
 *   => aforementioned CVars are only used in this function
 */
