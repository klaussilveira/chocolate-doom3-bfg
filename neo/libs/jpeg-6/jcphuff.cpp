...
#define IRIGHT_SHIFT(x, shft) \
    ((ishift_temp = (x)) < 0 ? (ishift_temp >> (shft)) | ((~0) << (16 - (shft))) : (ishift_temp >> (shft)))
#else
#define ISHIFT_TEMPS
    ...
    if (cinfo->Ah == 0) {
        ...
        if (is_DC_band) {
            ...
            if (entropy->bit_buffer == NULL) {
                entropy->bit_buffer = (char*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        if (gather_statistics) {
            ...
            if (entropy->count_ptrs[tbl] == NULL) {
                entropy->count_ptrs[tbl] = (long*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
                    257 * SIZEOF(long));
            }
...
jinit_phuff_encoder(j_compress_ptr cinfo)
{
    ...
    entropy = (phuff_entropy_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
