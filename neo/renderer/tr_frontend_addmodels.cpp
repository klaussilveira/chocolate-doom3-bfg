...
bool R_IssueEntityDefCallback(idRenderEntityLocal* def)
{
    ...
    if (r_checkBounds.GetBool()) {
        if (oldBounds[0][0] > def->localReferenceBounds[0][0] + CHECK_BOUNDS_EPSILON || oldBounds[0][1] > def->localReferenceBounds[0][1] + CHECK_BOUNDS_EPSILON || oldBounds[0][2] > def->localReferenceBounds[0][2] + CHECK_BOUNDS_EPSILON || oldBounds[1][0] < def->localReferenceBounds[1][0] - CHECK_BOUNDS_EPSILON || oldBounds[1][1] < def->localReferenceBounds[1][1] - CHECK_BOUNDS_EPSILON || oldBounds[1][2] < def->localReferenceBounds[1][2] - CHECK_BOUNDS_EPSILON) {
...
idRenderModel* R_EntityDefDynamicModel(idRenderEntityLocal* def)
{
    ...
    if (def->dynamicModel == NULL) {
        ...
        if (def->cachedDynamicModel != NULL && r_checkBounds.GetBool()) {
            ...
            if (b[0][0] < def->localReferenceBounds[0][0] - CHECK_BOUNDS_EPSILON || b[0][1] < def->localReferenceBounds[0][1] - CHECK_BOUNDS_EPSILON || b[0][2] < def->localReferenceBounds[0][2] - CHECK_BOUNDS_EPSILON || b[1][0] > def->localReferenceBounds[1][0] + CHECK_BOUNDS_EPSILON || b[1][1] > def->localReferenceBounds[1][1] + CHECK_BOUNDS_EPSILON || b[1][2] > def->localReferenceBounds[1][2] + CHECK_BOUNDS_EPSILON) {
...
void R_AddSingleModel(viewEntity_t* vEntity)
{
    ...
    if (renderEntity->hModel == NULL || renderEntity->hModel->ModelHasInteractingSurfaces() || renderEntity->hModel->ModelHasShadowCastingSurfaces()) {
    ...
    for (int surfaceNum = 0; surfaceNum < model->NumSurfaces(); surfaceNum++) {
        ...
        for (int contactedLight = 0; contactedLight < numContactedLights; contactedLight++) {
            ...
            if (lightDef->parms.prelightModel && lightDef->lightHasMoved == false && entityDef->parms.hModel->IsStaticWorldModel() && !r_skipPrelightShadows.GetBool()) {
