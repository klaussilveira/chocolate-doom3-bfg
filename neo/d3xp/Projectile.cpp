...
void idProjectile::Create(idEntity* owner, const idVec3& start, const idVec3& dir)
{
    ...
    if (*(const char*)shaderName) {
        ...
        renderLight.lightRadius[0] = renderLight.lightRadius[1] = renderLight.lightRadius[2] = spawnArgs.GetFloat("light_radius");
#ifdef ID_PC
...
void idProjectile::Explode(const trace_t& collision, idEntity* ignore)
{
    ...
    if (*light_shader) {
        ...
        renderLight.pointLight = true;
        renderLight.lightRadius[0] = renderLight.lightRadius[1] = renderLight.lightRadius[2] = spawnArgs.GetFloat("explode_light_radius");
#ifdef ID_PC
        ...
        // Midnight ctf
        if (gameLocal.mpGame.IsGametypeFlagBased() && gameLocal.serverInfo.GetBool("si_midnight")) {
            renderLight.lightRadius[0] = renderLight.lightRadius[1] = renderLight.lightRadius[2] = spawnArgs.GetFloat("explode_light_radius") * 2;
        }
...
void idBFGProjectile::Spawn()
{
    ...
    if (temp != NULL && *temp != '\0') {
        ...
        secondModel.shaderParms[SHADERPARM_RED] = secondModel.shaderParms[SHADERPARM_GREEN] = secondModel.shaderParms[SHADERPARM_BLUE] = secondModel.shaderParms[SHADERPARM_ALPHA] = 1.0f;
...
void idBFGProjectile::Think()
{
    if (state == LAUNCHED) {
        ...
        for (int i = 0; i < beamTargets.Num(); i++) {
            ...
            beamTargets[i].renderEntity.shaderParms[SHADERPARM_RED] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_GREEN] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_BLUE] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_ALPHA] = 1.0f;
            if (gameLocal.time > nextDamageTime) {
                ...
                if (damageFreq && *(const char*)damageFreq && beamTargets[i].target.GetEntity() && (forceDamage || beamTargets[i].target.GetEntity()->CanDamage(GetPhysics()->GetOrigin(), org))) {
                    ...
                    beamTargets[i].renderEntity.shaderParms[SHADERPARM_RED] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_GREEN] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_BLUE] = beamTargets[i].renderEntity.shaderParms[SHADERPARM_ALPHA] = 0.0f;
                    bfgVision = false;
                }
                if (player) {
