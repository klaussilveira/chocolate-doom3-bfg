...
bool idMultiplayerGame::PointLimitHit()
{
    ...
    if (teamPoints[0] >= pointLimit || teamPoints[1] >= pointLimit)
        return true;
...
void idMultiplayerGame::GetSpectateText(idPlayer* player, idStr spectatetext[2], bool scoreboard)
{
    ...
    if (player->wantSpectate && !scoreboard) {
        if (gameLocal.gameType == GAME_LASTMAN && gameState == GAMEON) {
            ...
            if (gameLocal.gameType == GAME_TOURNEY && (currentTourneyPlayer[0] != -1 && currentTourneyPlayer[1] != -1) && (currentTourneyPlayer[0] != player->entityNumber && currentTourneyPlayer[1] != player->entityNumber)) {
