...
void idPlayer::ServerSpectate(bool spectate)
{
    ...
    if (spectate && common->IsMultiplayer() && gameLocal.mpGame.IsGametypeFlagBased() && carryingFlag) {
        DropFlag();
    }
}
...
void idPlayer::UpdateLaserSight()
{
    ...
    if (!IsGameStereoRendered() || !weapon.GetEntity()->ShowCrosshair() || AI_DEAD || weapon->IsHidden() || !weapon->GetMuzzlePositionWithHacks(muzzleOrigin, muzzleAxis)) {
        // hide it
